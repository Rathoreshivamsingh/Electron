...
class App {
    constructor() {
        ...
        this.isQuitting = false;  // Add this line
        
        this.init();
        this.setupIpcHandlers();  // Add this line
    }

    // setupIpcHandlers() {
    //     electron.ipcMain.on('close-window', async (event) => {
    //         try {
    //             const win = electron.BrowserWindow.fromWebContents(event.sender);
    //             if (win) {
    //                 this.isQuitting = true;  // Set quitting flag
    //                 await this.cleanup();
    //                 win.close();
    //             }
    //         } catch (error) {
    //             log.error('Error handling close-window event:', error);
    //         }
    //     });
    // }

    setupIpcHandlers() {
        electron.ipcMain.on('close-window', async () => {
            try {
    ...
    async createWindow() {
        try {
            ...
            // Add this before loadFile
            this.mainWindow.on('close', async (e) => {
                if (!this.isQuitting) {
                    e.preventDefault();
                    this.mainWindow.webContents.send('show-exit-prompt');
                }
            });
    ...
    async cleanup() {
        ...
        try {
            ...
            // Force kill any remaining Python or Orthanc processes
            if (process.platform === 'win32') {
                try {
                    childProcess.execSync('taskkill /F /IM python_server.exe');
                    childProcess.execSync('taskkill /F /IM Orthanc.exe');
                } catch (e) {
                    log.info('No leftover processes to kill');
                }
            }
